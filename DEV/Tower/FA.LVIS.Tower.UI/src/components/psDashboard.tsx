...
const DashboardComponent = () => {
  ...
  const getCurrentUser = async () => {
    try {
      const response = await axios.get('/user/info'); // Adjust the endpoint as needed
  ...
  const setAccessRights = (userInfo) => {
    const { CanManageTEQ, CanManageBEQ, ActivityRight } = userInfo;
  ...
  return (
    <div>
      <h1>Dashboard</h1>
      {/* Render your UI components based on the state */}
    </div>
  );
};
