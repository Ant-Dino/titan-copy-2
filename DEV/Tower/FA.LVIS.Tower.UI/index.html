...
const App = () => {
  ...
  const [activityRight, setActivityRight] = useState<string>(''); // Example value could be 'Admin'
  const [canManageAccessReq, setCanManageAccessReq] = useState<boolean>(false);
  useEffect(() => {
    // Here you would fetch or calculate your states such as currentUser, tenantName, etc.
    // Example:
    setCurrentUser('John Doe');
    setTenantName('Some Tenant');
    setUnauthorizedError(''); // Assuming this comes from some auth check
